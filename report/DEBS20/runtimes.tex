...
For the sake of comparisons and fairness of benchmarking, we are interested in comparing runtimes and programming languages supported by multiple cloud providers.  
...
Python \vs{MAISSEN: which version exactly?} is similarly well supported, except by Azure with Windows \gls{OS}.
Microsoft's \texttt{.NET Core} lacks support by Google \vs{MAISSEN: add missing details, we cannot say$\rightarrow$} and so forth. 
In the reminder of this paper, including our evaluation of \sys, we focus our comparison on the top four best supported runtimes. 
