...
Additionally, providers can apply finer-grain billing models (\emph{e.g.}, on a per service-call basis) for any of the offered services~\cite{serverlessaws, serverlessazure}, leading to reduced costs for developers.
...
We distinguish between different types of serverless options: \emph{(1)} FaaS (Function as a Service, \emph{e.g.}, \gls{AWS} Lambda~\cite{AWSLambda}, \emph{(2)} DBaaS (Database as a Service, \emph{e.g.} Microsoft Azure Database for PostgreSQL~\cite{AzureDBaaS}) and \emph{(3)} STaaS (Storage as a service, \emph{e.g.}, Google Cloud Storage~\cite{serverlessgoogle}). 
...
Despite the convenience of the approach, we observe how several open questions remain unsolved. %some issues remain unresolved and some questions stay open. 
Namely, it is currently hard to decide on a specific \gls{FaaS} provider based on criteria such as performance, workload adaptability or costs.
...
%With this suite 
In a nutshell, application developers can use the suite to automatically deploy several tests, to observe the performance of different cloud providers according to the chosen runtime. 
