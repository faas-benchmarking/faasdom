...
We distinguish between: \emph{(1)} FaaS (Function as a Service, \emph{e.g.}, \gls{AWS} Lambda~\cite{AWSLambda}, \emph{(2)} DBaaS (Database as a Service, \emph{e.g.} Microsoft Azure Database for PostgreSQL~\cite{AzureDBaaS}) and \emph{(3)} STaaS (Storage as a service, \emph{e.g.}, Google Cloud Storage~\cite{serverlessgoogle}). 
