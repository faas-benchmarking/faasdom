...
Note that this benchmark was performed from Bern, Switzerland, hence different results might be achieved from different locations. 
...
On AWS and IBM, cold starts can be deteced with the help of instance IDs. 
On Azure and Google, online metrics of running instances were taken into account to verify cold starts. 
The challenge of cold start detection lead to a manual test execution and data manipulation.
...
\begin{table}[!t]
    \caption{Pricing calculation: cost of the CPU-bound benchmarks computed by \sys according to the pricing models of each cloud. Execution times are rounded up to the next 100\,ms for AWS, Google and IBM~\cite{AWSPricing,GoogleFunctionsPricing, IBMPricing}\label{table:example2}}.
    \setlength{\tabcolsep}{3pt}
  \scriptsize
  \center
  \rowcolors{1}{gray!10}{gray!0}
