...
There are several steps to perform before a function can execute: allocate a server, setup a worker with the associated code, packages and extensions, load the function in memory, and finally run it.
...
On Google, this time varied varied from 10 minutes up to 10 hours. %\mais{I said to you 12 hours as mentioned previously in this section. I meant also 10. I corrected this, you can remove this comment.}
...
We report results for Python (Figure~\ref{fig:general_python_plot}), Node.js (Figure~\ref{fig:general_node_plot}), Go (Figure~\ref{fig:general_go_plot}) and .NET (Figure~\ref{fig:general_dotnet_plot}).
...
When reaching 1000 requests per second, only a total of for .NET only 12 instances to serve .NET functions. % have been deployed (see figure \ref{fig:live_metrics_stream}). 
...
Table~\ref{table:example2} shows the costs computed by \sys.\vs{MAISSEN: how much do they differ from the real costs - those actually charged? do we know?} \mais{No I don't know. Free tier and the low quantity did not allow to calculate that exactly. This is just an approximation.}
...
Azure strikes out to be very cheap. Since this example function application only uses 100 \gls{MB} Azure also only charges for 128 \gls{MB}.  \vs{MAISSEN: why only 1 row of results for Azure ? } \mais{Because memory is allocated and billed dynamically up to the needed next 128MB step. In this example the functions uses only 100 MB (because it is CPU bound) and Azure bills 128MB. Clear?}
...
\textbf{AWS Lambda.} Overall, it was the best-performing cloud provider. 
...
The cold start latency of 2s~4s makes it not suitable for short-lived sessions, leading to increased costs \mais{cost should not be higher with cold starts. Clouds only bill when the function is actually running / executing code} especially under heavy workloads.
...
\textbf{Google Cloud Functions.} The Google-based offering leverages a simple and clearly structured web-portal, as well a a consistent \gls{CLI} tool. 
...
\vs{MAISSEN: anything else we could mention here?} \mais{maybe that only 3 runtimes are supported, pricing calculated more granularly which makes it hareder for a user to estimate / understand, not that many configuration / control over the service (but this can be seen as good or bad)}
