...
\vs{MAISSEN: can you confirm I understood correctly the results in table 3?}
...
Table~\ref{table:example2} shows the costs computed by \sys.\vs{MAISSEN: how much do they differ from the real costs - those actually charged? do we know?}
...
Azure strikes out to be very cheap. Since this example function application only uses 100 \gls{MB} Azure also only charges for 128 \gls{MB}.  \vs{MAISSEN: why only 1 row of results for Azure ? }
