...
We executed these tests between \vs{MAISSEN: insert start date} and \vs{MAISSEN: insert end date}.
...
\caption{Latency test scatter plot. At the tiem of our experiment, \texttt{azure-west-us}  had issues deploying Node.js, hence re resort to \texttt{.NET}.  \vs{MAISSEN: Remove - Bern, Switzerlad (it discloses authors location, against double-blind); it'd be better to have the x-tics rotated a bit, see email}}
...
\begin{figure}[htp]
\begin{center}
\includegraphics[width=0.5\textwidth]{bilder/azure-coldstart.jpeg}
