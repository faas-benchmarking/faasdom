...
With the exception of \textit{aws-australia-east} \mais{This was Azure not AWS, so should be Azure australiaeast}, we deploy Linux \vs{MAISSEN: version/distribution} \mais{AWS: Amazon Linux 2, Image: Custom, Kernel: 4.14.138-99.102.amzn2.x86\_64; see \url{https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html}} \mais{Azure, Google and IBM do not mention anything about the OS. Unfortunately that did not come to my mind when testing it. I can try to find things out by executing commands like 'uname -a'. Please let me know if I shall do that.}.   
...
This occurs mostly after the deployment, when the function has not been used in a while\vs{MAISSEN: is it know how long is this period?} \mais{It is normally around 10 minutes of no invocation for an instance, but can also be higher. E.g. Google had an instance up for about 12h before disallocating it.} or when a new instance needs to be provisioned for scaling purposes. 
...
\gls{IBM} shows a similar pattern \vs{MAISSEN: can you clarify? you mean that the ranking of the results wrt to the languages is the same?} \mais{Yes that's what I meant, similar ranking/pattern} as \gls{AWS} although the cold start latency is around 600 ms higher for Node.js, Python and Go but similar for .NET.
...
On \gls{AWS} Lambda, the standard deviation is very low (\vs{MAISSEN: how much is the lowest?} \mais{lowest overall is 50 ms for 2048Mb .NET}), with very consistent execution times. 
