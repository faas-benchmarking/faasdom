...
Its goal is to calculates the cost \vs{MAISSEN: the cost of what?} for the supported cloud providers, either by setting the corresponding parameters manually or by exploiting results from previous executions.
...
\textbf{Runtimes (Node.js, Go, .NET):} In addition to the source code\vs{MAISSEN: you mean the source code of the function(s) to execute?}, a built and packaged zip file is needed for the deployment on all cloud except for Google. 
...
The deployment process is parallelized, \vs{MAISSEN: can you detail a bit more what exactly is parallelized?}.
...
\vs{In the appendix \ref{chapter:flow_charts}, flow charts are illustrating the deployment and cleanup process for each cloud.}
...
\vs{Since the main application runs on Docker and invokes other containers which also run in Docker, the main container needs access to \texttt{/var/run/docker.sock} \emph{i.e.} mounting it as volume. 
Otherwise a container cannot run or start another container for security and integrity reasons. This concept is called Docker-in-Docker and should generally not be used and is more of a hack than a feature. In this case it serves a simple purpose and does not implicate any issues.}