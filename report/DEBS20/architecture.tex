...
Its goal is to calculate the cost \mais{OK: of executing a certain amount of functions on} the supported cloud providers, either by setting the corresponding parameters manually or by exploiting results from previous executions.
...
\textbf{Runtimes (Node.js, Go, .NET):} In addition to the source code\vs{MAISSEN: you mean the source code of the function(s) to execute?} \mais{Yes}, a built and packaged zip file is needed for the deployment on all cloud except for Google. 
...
The deployment process is parallelized, \vs{MAISSEN: can you detail a bit more what exactly is parallelized?} \mais{OK?: meaning creating the required resources on the cloud, building and packaging the functions, deploying/uploading them}.
\vs{MAISSEN: I've looked a the flowcharts in the appendix of the report. Apart from the google one which seems very simplistic, I wonder if we could have a figure combining those ones into a figure that we could report here in the deployment section. wdyt?}
\vs{In the appendix \ref{chapter:flow_charts}, flow charts are illustrating the deployment and cleanup process for each cloud.} \mais{Yes I can try to combine and minify them. Maybe I forget about the runtime differences (build, zip) and only illustrate the essentail 'cloud' part?}

\subsection{Cleanup}
Upon cleanup, all deployed Lambda functions are permanently deleted, as well as all the configurations and resource groups created at deployment time. % and \gls{API} gateways on \gls{AWS} and IBM, all resource groups with a name containing latency, factors, matrix, filesystem and custom and on Google all functions in the configured project. 
...
Our design is modular and currently supports \sloppy{\texttt{wrk2}~\cite{wrk2}}, a constant throuput/fixed latency benchmarking tool. 
...
\subsection{Billing Costs Prospection}
\vs{MAISSEN: this component is not in the architecture drawing? if so, where does it fit?} \mais{It belongs to the main app node app.}
