x <- read.csv("percentile_latency_dotnet.csv",header=T,sep=",")
x <- t(x)
...
azure_win_node_50 <- as.numeric(x[8,1:32])
azure_win_node_95 <- as.numeric(x[9,1:32])
azure_win_node_99 <- as.numeric(x[10,1:32])
ibm_node_50 <- as.numeric(x[11,1:32])
ibm_node_95 <- as.numeric(x[12,1:32])
ibm_node_99 <- as.numeric(x[13,1:32])
png(file = "plot_percentile_latency_dotnet.png", width=1200, height=700, units="px", pointsize=24)
par(mar=c(4,5,2,0)+.1)
plot(aws_node_50, type="l", lwd=2, lty=1, col = "#E69F00", xlab = expression(bold("requests per second")), ylab = "", main = ".NET percentiles 50% - 95% - 99%",ylim=c(0,30000), xaxt="n", las=1, frame=FALSE)
...
legend(1, 20000, legend=c("AWS", "Azure", "Azure (Win)", "IBM", "99%", "95%", "50%"),
col=c("#E69F00", "#56B4E9", "purple", "#E33939", "black", "black", "black"), lwd=2,
